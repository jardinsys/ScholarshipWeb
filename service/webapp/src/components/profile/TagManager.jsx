...
/**
 * TagManager — lets a logged-in user view, add, and remove their tags.
 * Replaces the browser prompt() with a proper inline input form.
 */
export function TagManager({ userId, tags, onTagsChange }) {
  const [allTags, setAllTags]           = useState([])
  const [search, setSearch]             = useState('')
  const [showPicker, setShowPicker]     = useState(false)
  const [adding, setAdding]             = useState(null)   // tag._id currently being added
  const [pendingTag, setPendingTag]     = useState(null)   // tag awaiting value input
  const [pendingValue, setPendingValue] = useState('')
  const [valueError, setValueError]     = useState('')
  const valueInputRef                   = useRef(null)
  ...
  useEffect(() => {
    if (pendingTag && valueInputRef.current) {
  ...
  // Step 1: user clicks a tag type → show value input
  const handleSelectTag = (tag) => {
    setPendingTag(tag)
    setPendingValue('')
    setValueError('')
  ...
  // Step 2: user submits the value → call API
  const handleConfirmAdd = async () => {
    ...
    <div className="animate-fade-up delay-150 space-y-4">
      ...
      {showPicker && (
        <div className="rounded-xl border border-border bg-white shadow-md p-4 space-y-3">
          ...
          {/* Value entry form (shown after selecting a tag type) */}
          {pendingTag ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={adding === pendingTag._id}
                >
                  {adding === pendingTag._id
                    ? <><Loader2 size={12} className="animate-spin" /> Saving…</>
                    : 'Add Tag'}
